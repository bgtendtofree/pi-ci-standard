...
- GitHub Actions runs checks through managed mise tasks, never language-specific workflow commands
...
Same core logic (init/audit only) is exposed as a CLI for agents/scripts via bash:
...
- `audit` — detailed read-only configuration report, nonzero exit on any finding. It shows toolchain pins, command mode, platform runners, and managed workflow runners. It does not run project checks; use `mise run check` or `mise run ci` for execution.
...
## Project-specific commands and platforms

Go and Rust projects may replace the generated check commands and add focused platform verification with `.pi-ci.json`:
...
`check` and `ci` become the corresponding managed mise task commands. `platform` adds a managed `platform-check` task and a runner-matrix job. Keep platform checks focused; platform-independent lint, coverage, and vulnerability checks belong in the main CI command. Node projects already own customization through their required `validate` and `ci` package scripts.

Audit marks a platform command that exactly matches `check` or `ci` for duplicate-execution review. `/ci fix` invokes the agent for that review even when the structural audit passes; once the platform command is distinct, a passing fix remains a no-op.

## Prerequisites

`init` fails mutation-free when any prerequisite is absent; `audit` reports values and failures. pi-ci never installs dependencies, writes versions, adds scripts, or creates biome config.
...
- `.pi-ci.json` is project-owned, strictly validated, and never rewritten.
